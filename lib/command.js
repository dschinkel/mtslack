...
const { getTmpDir, isLinux, isSnap } = require('./dirs');
...
const unmount = () => {
  bottomBar.log.write(chalk.grey('Unmounting Snap application...'));
  spawn('killall', ['slack'], { stdio: 'ignore' });
  spawn('umount', [`/snap/slack/current/usr/lib/slack/resources/app.asar`], { stdio: 'ignore' });
};

const mount = (source, archive) => {
  bottomBar.log.write(chalk.grey('Remounting Snap application...'));
  spawn('mount', ['--bind', '-o', 'nodev,ro', source, archive]);
};

const cleanup = () => {
  bottomBar.log.write(chalk.grey('Cleaning up tmp dir...'));
  rimraf.sync(getTmpDir());
};
...
exports.apply = async (archive) => {
  ...
  if (isLinux() && isSnap(archive)) {
    unmount();
  }

  const prefix = getTmpDir();
  if (!fs.existsSync(prefix)) {
    fs.mkdirSync(prefix);
  }
  // Make a backup
  fs.copyFileSync(archive, `${prefix}/app.asar.backup`);
  // Extract archive
  asar.extractAll(archive, `${prefix}/app.asar.unpacked`);
  // Add js code and recreate the package
  fs.appendFileSync(`${prefix}/app.asar.unpacked/dist/preload.bundle.js`, generateJS());
  await asar.createPackage(`${prefix}/app.asar.unpacked`, `${prefix}/app.asar`);

  // if (isLinux() && isSnap(archive)) {
  //   mount(`${prefix}/app.asar`, archive);
  //   // mount(`${prefix}/app.asar.backup`, `${archive}.backup`);
  // }
...
exports.remove = async (archive) => {
  const prefix = getTmpDir();
  if (!fs.existsSync(`${prefix}/app.asar.backup`)) {
    return;
  }

  if (isLinux() && isSnap(archive)) {
    unmount();
    mount(`${prefix}/app.asar.backup`, archive);
  }

  // fs.copyFileSync(`${prefix}/app.asar.backup`, archive);
